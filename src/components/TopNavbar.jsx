...
const TopNavbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Since hero is h-screen (100vh), show navbar when scrolled past 90% of viewport
      // This gives a nice transition as the hero section starts to exit
      const heroThreshold = viewportHeight * 0.9
      const shouldShow = scrollY >= heroThreshold
    ...
    <>
      <style>{`
        @keyframes topNavSlideIn {
          0% {
            opacity: 0;
            transform: translateX(-50%) translateY(-100px) scale(0.5) rotateX(-30deg);
            filter: blur(10px);
            box-shadow: 6px 6px 0 #ff8c42, 0 0 80px rgba(255, 140, 66, 1);
          }
          60% {
            opacity: 0.8;
            transform: translateX(-50%) translateY(10px) scale(1.1) rotateX(5deg);
            filter: blur(3px);
            box-shadow: 8px 8px 0 #ff8c42, 0 0 60px rgba(255, 140, 66, 0.8);
          }
          100% {
            opacity: 1;
            transform: translateX(-50%) translateY(0px) scale(1) rotateX(0deg);
            filter: blur(0px);
            box-shadow: 6px 6px 0 #ff8c42, 0 0 40px rgba(255, 140, 66, 0.4);
          }
        }
      `}</style>
      <nav style={topNavContainer}>
        <div style={navList}>
          {navItems.map((item) => {
